...
public class IbAdaptor implements EWrapper, IMarketDataAdaptor, IStreamAdaptor<IDownStreamConnection> {
	...
	public void subscribeMarketData(String instrument,
		...
		IbRefData refData = (IbRefData) refDataManager.getRefData(instrument);
		if(refData == null) {
		...
		clientSocket.reqMktData(reqId, refData.getContract(), genericTickTags, false);
		log.info("subscribeMarketData: " + instrument + "," + reqId);
		if(reqMarketDepth) {
			clientSocket.reqMktDepth( reqId, refData.getContract(), depthRows);
	...
	class DownStreamSender implements IDownStreamSender {
		...
		public void newOrder(ChildOrder order) throws DownStreamException {
			...
			IbRefData refData = (IbRefData) refDataManager.getRefData(order.getSymbol());
			if(refData == null) {
	        ...
	        clientSocket.placeOrder(orderId, refData.getContract(), ibOrder);
...
//			pendingAmends.put(orderId, fields);
	...
	public void tickPrice(int tickerId, int field, double price,
		...
		default:
			log.warn("tickPrice: " + symbol + " undefined field type: " + field);
		}
	}
	...
	public void tickSize(int tickerId, int field, int size) {
		...
		case 8: 
			...
			break;
		default:
			log.warn("tickSize: " + symbol + " undefined field type: " + field);
		}
	}
