...
import com.cyanspring.cstw.event.ParentOrderSelectionEvent;
import com.cyanspring.cstw.gui.common.ColumnProperty;
import com.cyanspring.cstw.gui.common.DynamicTableViewer;
import com.cyanspring.cstw.gui.filter.ParentOrderFilter;

public class ParentOrderView extends ViewPart implements IAsyncEventListener {
	private static final Logger log = LoggerFactory.getLogger(ParentOrderView.class);
	...
	private enum StrategyAction { Pause, Stop, Start };
	...
	public void createPartControl(final Composite parent) {
			...
			public void handleEvent(Event event) {
				...
				if (obj instanceof HashMap) {
					...
					Business.getInstance().getEventManager().
						sendEvent(new ParentOrderSelectionEvent(map, Business.getInstance().getSingleInstrumentAmendableFields(strategyName)));
				}
			}
		});
		...
		Business.getInstance().getEventManager().subscribe(GuiParentOrderUpdateEvent.class, this);
		showOrders();
	}
	...
	private void saveOrder() {
		...
		Table table = ParentOrderView.this.viewer.getTable();
...
//				if(titles.contains(field))
	...
	public void onEvent(AsyncEvent event) {
		if (event instanceof OrderCacheReadyEvent) {
			...
			smartShowOrders();
		} else if (event instanceof GuiParentOrderUpdateEvent) {
			log.debug("Recieved event: " + event);
			smartShowOrders();
